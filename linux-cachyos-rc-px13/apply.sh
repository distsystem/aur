#!/usr/bin/env bash
# Re-sync from CachyOS upstream linux-cachyos-rc/{PKGBUILD,config}, then
# re-apply the px13 customization on top. Called from ~/distsystem/aur/update-all.sh.
# Argument: new version reported by nvchecker (e.g. "7.1-rc3-2")
set -euo pipefail
cd "$(dirname "$(readlink -f "$0")")"
. "$(dirname "$PWD")/lib/bump.sh"

new_ver="${1:?usage: apply.sh <ver>}"   # e.g. 7.1-rc3-2 — informational

# 1. Pull fresh upstream PKGBUILD + config
gh api 'repos/CachyOS/linux-cachyos/contents/linux-cachyos-rc/PKGBUILD?ref=master' \
    --jq '.content' | base64 -d > PKGBUILD.new
gh api 'repos/CachyOS/linux-cachyos/contents/linux-cachyos-rc/config?ref=master' \
    --jq '.content' | base64 -d > config.new

# 2. Inject -px13 into _pkgsuffix on all 3 branches.
# Validate we hit 3 occurrences, otherwise upstream changed shape — abort.
n=$(grep -cE '^\s*_pkgsuffix="cachyos-rc[a-z-]*"' PKGBUILD.new)
if [[ $n -ne 3 ]]; then
...
sed -i -E 's/^(\s*_pkgsuffix=")(cachyos-rc[a-z-]*)(")/\1\2-px13\3/' PKGBUILD.new

# 3. Append px13 customization block from this script (kept in lockstep with PKGBUILD)
...
# 4. Diff against existing — if nothing changed, no-op
